...
// NOTE: 精讀註解版：由原始 PSUSensorMain.cpp 產生，只供閱讀，不要拿去編譯。
// NOTE: 原始檔未修改；本檔採用「區塊導讀 + 關鍵行 inline 註解」，避免每行重複空泛文字。
// NOTE: 主流程：EntityManager config + Linux sysfs hwmon/IIO -> PSUSensor D-Bus object -> bmcweb/IPMI 可讀。
...
// [導讀] 這一組 include 是 dbus-sensors 內部模組。
// [導讀] PSUSensorMain.cpp 本身不直接知道硬體暫存器，它主要靠 DeviceMgmt、Utils、PSUSensor、PSUEvent 把 EntityManager 設定與 sysfs 檔案串起來。
...
// [導讀] Boost/sdbusplus/phosphor-logging 是 OpenBMC daemon 常見組合：
// [導讀] Boost.Asio 負責 event loop/timer；sdbusplus 負責 D-Bus；lg2 負責 log。
...
// [導讀] 標準函式庫主要用於：filesystem 掃描 sysfs、fstream 讀 sysfs 檔、regex 解析檔名、variant 解析 EntityManager 設定。
#include <algorithm>
#include <array>
#include <cctype>
#include <chrono>
...
// [導讀] devRegex 用來從 hwmon 的 real device path 找出 I2C bus/address。
// [例] /sys/.../i2c-3/3-0058/... 會擷取出 /i2c-3/3-0058，後面再得到 bus=3 addr=0x58。
static std::regex devRegex(R"((\/i[23]c\-\d+\/\d+-[a-fA-F0-9]{4,4})(\/|$))");

// [導讀] sensorTypes 是 PSU sensor 白名單與 driver 對應表。
// [重點] 左邊是 EntityManager/config 使用的型別名稱；右邊第一個字串是 Linux driver/hwmon name；bool 表示是否需要 DeviceMgmt 管理 I2C device。
// [影響] 如果 hwmon name 不在這張表，createSensorsCallback 會跳過並印出 not found in sensor whitelist。
...
// [導讀] DevTypes 區分資料來源。HWMON 來自 /sys/class/hwmon；IIO 來自 /sys/bus/iio/devices。
enum class DevTypes
{
    Unknown = 0,
    HWMON,
...
// [導讀] DevParams 定義每種來源如何找檔案、如何從檔名解析 sensor 類型。
struct DevParams
{
...
// [導讀] 這三個全域 map 保存已建立的物件，避免 callback 重新掃描時物件生命週期消失。
...
// [導讀] SensorUnit 與 sensorTable 負責把 hwmon 類型轉成 OpenBMC D-Bus sensor 單位。
struct SensorUnit
{
...
// [導讀] labelMatch 是 label 前綴的預設屬性表。
// [例] vout -> Output Voltage；iout -> Output Current；pin -> Input Power；pout -> Output Power。
// [欄位] {hwmon label, 顯示名稱, max, min, scale 指數或除數, offset}。
...
// [導讀] eventMatch/limitEventMatch 把 sysfs alarm 檔案轉成 PSU 狀態事件。
// [例] power1_alarm 可能對應 PredictiveFailure；crit_alarm/lcrit_alarm 對應 Failure。
...
// [導讀] cpuPresence 記錄 inventory 裡 CPU 是否 present，給 CPURequired 設定使用。
static boost::container::flat_map<size_t, bool> cpuPresence;
// [導讀] devParamMap 是 HWMON/IIO 的檔名規則表。
// [HWMON] 找 in1_input、curr1_input、power1_input 這類檔案。
// [IIO] 找 in_voltage0_raw/input 這類檔案。
...
// [導讀] checkEvent 掃描固定 event 檔名是否存在。
// [流程] eventMatch 告訴它要找哪些檔；找到後存進 eventPathList，之後 PSUCombineEvent 會讀這些檔案判斷狀態。
...
// events.
// [導讀] checkGroupEvent 專門收集 fanN_alarm/fanN_fault。
// [目的] 多個 fan fault 會被合併成一個 FanFault 群組事件。
...
// to eventPathList.
// [導讀] checkEventLimits 由某個 xxx_input 檔案推導同前綴的 alarm 檔。
// [例] power1_input -> power1_crit_alarm / power1_lcrit_alarm / power1_max_alarm / power1_min_alarm。
...
// [導讀] checkPWMSensor 只處理 PSU fan。
// [條件] 若 fanN_input 有對應 fanN_target，代表可以建立 PWM target control。
...
// [導讀] createSensorsCallback 是本檔最核心函式。
// [大流程] EntityManager config + sysfs hwmon/IIO -> 配對 bus/address -> 解析 label/threshold -> 建立 PSUSensor D-Bus object。
...
// [導讀] instantiateDevices 先根據 EntityManager config 建立 I2CDevice 管理表。
    auto devices = instantiateDevices(sensorConfigs, sensors, sensorTypes);

// [導讀] pmbusPaths 其實保存的是 sysfs 裡所有 name 檔路徑。
// [流程] 後面會讀 name 檔內容，確認 driver/hwmon name 是否在 sensorTypes 白名單。
...
// [導讀] 開始逐一處理每個 sysfs device。每個 pmbusPath 通常像 /sys/class/hwmon/hwmonX/name。
    ...
    for (const auto& pmbusPath : pmbusPaths)
    {
        ...
        std::getline(nameFile, pmbusName); // 讀出 hwmon/IIO name，例如 pmbus、cffps、ina226。
        nameFile.close();

// [導讀] 白名單檢查：如果 name 檔讀到的 pmbusName 不在 sensorTypes，就不建立 sensor。
...
// [導讀] directory 是 name 檔所在資料夾，也就是後續掃描 input/label/alarm 的根目錄。
        auto directory = pmbusPath.parent_path(); // 取得 hwmonX/IIO device 目錄，後續所有 input/label/alarm 都從這裡找。
...
// [導讀] 預設此 device 是 HWMON。若不在 /sys/class/hwmon 下，後面會改成 IIO。
        DevTypes devType = DevTypes::HWMON;
        std::string deviceName;
// [導讀] HWMON path 需要透過 device symlink 找回真正 I2C 裝置位置。
        if (directory.parent_path() == "/sys/class/hwmon")
        {
            ...
            std::regex_search(devicePath, match, devRegex); // 從 real path 擷取 I2C bus/address 片段。
            if (match.empty())
            {
            ...
            deviceName = matchStr.substr(matchStr.find_last_of('/') + 1); // 留下 3-0058 這種格式給 getDeviceBusAddr。
        }
        else
        {
            deviceName =
...
// [導讀] 從 deviceName 解析出 bus/address 後，才能跟 EntityManager config 的 Bus/Address 配對。
        size_t bus = 0;
        size_t addr = 0;
        if (!getDeviceBusAddr(deviceName, bus, addr)) // 解析失敗就無法跟 EntityManager Bus/Address 配對。
        {
            continue;
        }

// [導讀] 以下開始用 sysfs 解析出的 bus/address 去 EntityManager config 裡找同一顆 PSU。
...
// [導讀] sensorConfigs 是 EntityManager 回傳的所有候選 config object；這裡逐一檢查。
...
// [導讀] Bus/Address 是配對關鍵。如果 config 沒寫這兩個欄位，就無法知道它對應哪個 sysfs device。
...
// [導讀] 只有 config 的 Bus/Address 跟 sysfs 裝置完全相同，才認定這份 config 是目前 PSU。
            if ((*confBus != bus) || (*confAddr != addr)) // config 不屬於目前 sysfs device，跳過。
            {
                lg2::debug(
            ...
            interfacePath = &path.str; // 找到匹配 config，保存 object path。
            break;
        }
...
// [導讀] 找到 config 後，用 interfacePath 回頭找 instantiateDevices 建好的 I2CDevice。
...
// [導讀] Name 是 PSU 名稱來源，後續會成為 sensor name 前綴與 event key。
...
// [導讀] CPURequired 表示這個 PSU/sensor 只有在某顆 CPU present 時才建立。
...
// [導讀] 若不是 first scan，代表是 config 變更觸發；此時只處理有變更的 PSU。
        // on rescans, only update sensors we were signaled by
        if (!firstScan)
        {
            std::string psuNameStr = "/" + escapeName(*psuName); // 把 PSU 名稱轉成 D-Bus path 形式以比對變更。
            auto it =
...
// [導讀] 這裡開始收集 PSU event/alarm 檔案，但真正 expose 狀態是在後面的 PSUCombineEvent。
        checkEvent(directory.string(), eventMatch, eventPathList);
        checkGroupEvent(directory.string(), groupEventPathList);

// [導讀] readState 決定 sensor 在 host on/off 等哪種 power state 下才讀值。
        PowerState readState = getPowerState(*baseConfig); // 讀取此 PSU sensor 的 power state 條件。

// [導讀] 支援同一 config interface 內有多個 Name 欄位：Name、Name1、Name2...
// [用途] hwmon label 的數字 index 會拿來配對 psuNames vector。
...
// [導讀] sensorPaths 會保存此 PSU 的所有數值檔，例如 in1_input、power1_input、temp1_input，也可能包含 *_max。
...
// [導讀] pollRate 控制 PSUSensor 多久讀一次 sysfs。未設定就用 PSUSensor::defaultSensorPoll。
        float pollRate = getPollRate(*baseConfig, PSUSensor::defaultSensorPoll);

// [導讀] Labels 是 config 裡的白名單。如果有設定，只有列出的 labelHead 會被 expose。
...
// [導讀] sensorNameRegEx 用來從檔名取出 sensor 類型，例如 in/curr/power/temp/fan。
        std::regex sensorNameRegEx(std::string(devParam->second.nameRegEx));
        std::smatch matches;

// [導讀] 以下進入單一 sensor 檔案處理流程。每個 sensorPath 最後可能建立一個 PSUSensor。
        for (const auto& sensorPath : sensorPaths)
        {
            ...
            if (std::regex_search(sensorNameStr, matches, sensorNameRegEx))
            {
                ...
                sensorNameSubStr = matches[param->second.matchIndex]; // 取得 sensor 類型，例如 in/curr/power/temp/fan。
            }
            else
            {
...
// [導讀] HWMON sensor 需要找對應 label 檔。沒有 label 時退回用檔名前綴當 labelHead。
            if (devType == DevTypes::HWMON)
            {
                ...
                else
                {
                    ...
                    // vin1, vout1, ...
                    labelHead = label.substr(0, label.find(' ')); // 取 label 第一段，例如 vout1。
                }

// [導讀] 若處理的是 *_max 檔，labelHead 前面加 max，才能匹配 maxpin/maxvin/maxiout 等 labelMatch。
                /* append "max" for labelMatch */
                if (maxLabel)
                {
                    labelHead.insert(0, "max"); // 把 vout1 變 maxvout1，用來匹配 max 類 label。
                }

// [導讀] 這裡做第一次 Labels 過濾，避免不需要的 label 連 PWM 都建立。
                ...
                checkPWMSensor(sensorPath, labelHead, *interfacePath, // fan 類 label 可能額外建立 PWM control。
                               dbusConnection, objectServer, psuNames[0]);
            }
// [導讀] IIO 不走 hwmon *_label 機制，所以直接從檔名組出 labelHead。
...
// [導讀] labelHeadView 是去掉數字的 label 前綴，例如 vout1 -> vout，用來查 labelMatch。
            auto it = std::find_if(labelHead.begin(), labelHead.end(),
                                   static_cast<int (*)(int)>(std::isdigit));
            std::string_view labelHeadView( // 去掉數字後查 labelMatch。
...
// [導讀] 複製 labelMatch 的預設值到本地變數，避免某顆 PSU 的客製化設定污染其他 PSU。
...
// [導讀] 以下組出每個 label 可覆寫的 config key。
// [例] vout1_Name、vout1_Scale、vout1_Min、vout1_Max、vout1_Offset、vout1_PowerState。
...
// [導讀] 客製化 Name 會改變命名策略：若有指定 Name，就不再自動加 PSU 名稱前綴。
            ...
            if (findCustomName != baseConfig->end())
            {
                try
                {
                    labelTypeName = std::visit(VariantToStringVisitor(), // 用 config 覆寫顯示名稱。
...
// [導讀] 客製化 Scale 時，程式會把該值直接當除數，不再做 10^N 轉換。
            ...
            if (findCustomScale != baseConfig->end())
            {
                try
                {
                    sensorScaleFactor = std::visit( // 用 config 覆寫 scale。
...
// [導讀] Min/Max/Offset 可由 config 覆蓋 labelMatch 預設值，用於平台校正或限制範圍。
...
// [導讀] 單一 label 也能覆寫 readState，例如某個 sensor 只在 host power on 時讀取。
            ...
            if (findPowerState != baseConfig->end())
            {
                ...
                setReadState(powerState, readState); // 用 label 專屬 PowerState 覆蓋整體 readState。
            }
...
// [導讀] 未客製化名稱時，程式會根據 labelHead 的數字決定配對哪個 PSU Name。
// [注意] hwmon label 數字是 1-based，但 psuNames vector 是 0-based，所以後面會減 1。
...
// [導讀] 只有 HWMON 來源有 threshold alarm sysfs 檔，因此只在 HWMON 做 checkEventLimits。
            if (devType == DevTypes::HWMON)
            {
                checkEventLimits(sensorPathStr, limitEventMatch, eventPathList);
            }

// [導讀] scale factor 處理分兩種：客製化 scale 直接使用；預設 scale 則用 10^N。
            ...
            if (!customizedScale)
            {
                ...
                factor = std::pow(10.0, factor); // 預設 scale 是 10 的指數。
...
// [導讀] 這裡解析單一 sensor 的 threshold，會依 labelHead 找對應設定。
...
// [導讀] 建立 D-Bus Sensor.Value 前，必須知道單位；sensorTable 找不到就跳過。
...
// [導讀] 以下組合最終 sensorName。這個名字會出現在 D-Bus sensor path/Redfish 顯示中。
            ...
            lg2::debug("Sensor name: {NAME}, path: {PATH}, type: {TYPE}",
                       ...
                       sensorType);
// [導讀] 如果不是 activateOnly，會先清掉舊 sensor，確保 config 變更後能重新建立。
            ...
            {
// [導讀] 建立 PSUSensor 的地方：此物件會讀 sensorPathStr 指向的 sysfs 檔，並 expose D-Bus sensor。
                sensors[sensorName] = std::make_shared<PSUSensor>( // 建立真正的 sensor object。
                    sensorPathStr, sensorType, objectServer, dbusConnection, io,
                    sensorName, std::move(sensorThresholds), *interfacePath,
                    readState, findSensorUnit->units, factor, maxReading,
                    minReading, sensorOffset, labelHead, thresholdConfSize,
                    pollRate, i2cDev);
                sensors[sensorName]->setupRead(); // 開始週期讀 sysfs 並更新 D-Bus。
                ++numCreated;
                lg2::debug("Created '{NUM}' sensors so far", "NUM", numCreated);
            }
        }

// [導讀] 每顆 HWMON PSU 會額外建立 OperationalStatus event，把前面收集的 alarm/fault 檔轉成狀態。
        if (devType == DevTypes::HWMON)
        {
            // OperationalStatus event
            combineEvents[*psuName + "OperationalStatus"] = nullptr;
            combineEvents[*psuName + "OperationalStatus"] =
                std::make_unique<PSUCombineEvent>( // 建立 PSU 狀態事件聚合器。
...
// [導讀] getPresentCpus 啟動時查一次 CPU inventory，避免只等後續 signal。
...
// [導讀] createSensors 是外層包裝：它先向 EntityManager 取得 config，拿到後才呼叫 createSensorsCallback。
void createSensors(
    boost::asio::io_context& io, sdbusplus::asio::object_server& objectServer,
    std::shared_ptr<sdbusplus::asio::connection>& dbusConnection,
    const std::shared_ptr<boost::container::flat_set<std::string>>&
...
// [導讀] powerStateChanged 負責 host power state 變化時啟停 sensor。
// [進入狀態] newState=true 時 createSensors(... activateOnly=true)。
// [離開狀態] newState=false 時把 readState 符合的 sensor deactivate。
...
// [導讀] main 是 psusensor daemon 入口：建立 D-Bus service、註冊 signal handler、排程初次掃描，最後 io.run 常駐。
int main()
{
    ...
    systemBus->request_name("xyz.openbmc_project.PSUSensor"); // 註冊 D-Bus name，systemd Type=dbus 會等待這個 name。
    auto sensorsChanged =
        std::make_shared<boost::container::flat_set<std::string>>();

// [導讀] sensorsChanged 用來 debounce EntityManager config 變更；3 秒內多個變更會合併處理。
    ...
    boost::asio::post(io, [&]() {
        createSensors(io, objectServer, systemBus, nullptr, false); // 初次全量建立 PSU sensors。
    });
// [導讀] 初次掃描用 boost::asio::post 排入 event loop，避免初始化尚未完成就開始建立 sensor。
    boost::asio::steady_timer filterTimer(io);
    std::function<void(sdbusplus::message_t&)> eventHandler =
        [&](sdbusplus::message_t& message) {
// [導讀] eventHandler 監聽 EntityManager PropertiesChanged。收到變更後等 3 秒再重建，避免短時間連續變更造成重複掃描。
            sensorsChanged->insert(message.get_path()); // 記錄哪個 EntityManager config path 改變。
            filterTimer.expires_after(std::chrono::seconds(3)); // 3 秒 debounce config 變更。
            filterTimer.async_wait([&](const boost::system::error_code& ec) {
                ...
                createSensors(io, objectServer, systemBus, sensorsChanged, // 只重建變更相關的 PSU sensors。
                              false);
            });
        };
    ...
    std::function<void(sdbusplus::message_t&)> cpuPresenceHandler =
        [&](sdbusplus::message_t& message) {
// [導讀] cpuPresenceHandler 監聽 CPU inventory Present 變化。CPU 變成 present 後延遲 1 秒重建 sensors。
            ...
            try
            {
                cpuPresence[index] = std::get<bool>(findPresence->second); // 更新 CPU present 狀態。
            }
            ...
            cpuFilterTimer.expires_after(std::chrono::seconds(1)); // CPU present 後等 1 秒再重建 sensor。
...
// [導讀] setupPropertiesChangedMatches 監聽所有 sensorTypes 對應 config 的 PropertiesChanged。
    std::vector<std::unique_ptr<sdbusplus::bus::match_t>> matches =
        setupPropertiesChangedMatches(*systemBus, sensorTypes, eventHandler); // 監聽 EntityManager sensor config 變更。

// [導讀] 這個額外 match 專門監聽 CPU inventory path 下的 Present 變更。
...
// [導讀] 啟動後先查一次 CPU presence，註冊 manufacturing mode，最後進入 event loop。
    getPresentCpus(systemBus); // 啟動時先建立 CPU presence 初始值。

    setupManufacturingModeMatch(*systemBus);
    io.run(); // 進入常駐 event loop。
}
